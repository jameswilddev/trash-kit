  ...
  const typeSeparated = diff.separate({
    engine: path => {
      ...
      if (match === null) {
        ...
        return {
          path,
          name: assembleName(match[1] as string, match[2] as string),
          extension: match[2] as string
        }
      }
    },
    game: path => {
      ...
      if (srcMatch !== null) {
        const srcFile: types.GameSrcFile = {
          type: 'src',
          path,
          game: srcMatch[1] as string,
          name: assembleName(srcMatch[2] as string, srcMatch[3] as string),
          extension: srcMatch[3] as string
        }
      ...
      if (metadataMatch !== null) {
        const metadataFile: types.GameMetadataFile = {
          type: 'metadata',
          path,
          game: metadataMatch[1] as string
        }
